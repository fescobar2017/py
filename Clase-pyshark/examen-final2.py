    ...
    for pkt in capture:
        try:
            ...
            if hasattr(pkt,'dns') and hasattr(pkt.dns,'qry_name'):
                 ...
                 if dns in listadns:
                    print(f"Trafico detectado hacia {dns} ")
                    trafico2.append([iporigen,dns])
...
def funcion3():
    ...
    trafico2 =[]
    capture = pyshark.FileCapture('pesadilla.pcapng',display_filter='dns or ip.dst==69.163.33.82 or ip.dst==87.106.139.101 or ip.dst==181.113.229.139 or ip.dst==159.65.241.220')
    #capture = pyshark.FileCapture('pesadilla.pcapng')
    for pkt in capture:
        try:
            if hasattr(pkt,'ip') and hasattr(pkt.ip,'src'):
                   iporigen = pkt.ip.src
...
while True:
    print (''' Menu 
        (1) Capturar tráfico DNS y TCP FIN-ACK, guardar en CSV         )
        (2) Cargar captura y filtrar por TCP o UDP hacia IPs sospechosas, guardar con Flags en CSV
        (3) Cargar captura y filtrar por TCP o UDP hacia IPs sospechosas (solo mostrar)
        (4) Capturar paquetes UDP (que no sean DNS), guardar en CSV"
        (5) Salir ''')      
